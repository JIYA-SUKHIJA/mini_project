/**
 * TaskFlow - Core Data Lifecycle & Dynamic Activity Controller
 * Coordinates filtering pipelines, task actions, and the dynamic activity timeline.
 */
...
const defaultTasks = [
    ...
    { id: "task-3", title: "Client Follow-ups", desc: "Follow up with Max Life Insurance client leads for policy renewals.", priority: "Medium", date: "2026-07-05", status: "Pending" },
    { id: "task-4", title: "Finalize VLM Review Paper", desc: "Complete the 40-page draft on Architectural Innovations and Optimization Frontiers in Compact Vision-Language Models.", priority: "High", date: "2026-07-15", status: "Pending" }
];
...
document.addEventListener('DOMContentLoaded', () => {
    renderTaskBoard();
    renderActivityTimeline(); // Hydrate the timeline template on initialization
    repositionIndicator();
...
if(clearFiltersBtn) {
    clearFiltersBtn.addEventListener('click', () => {
        searchFilterStr = ""; priorityFilterStr = ""; statusFilterStr = "all";
...
function updateProgressCircleTrack(percentage) {
    ...
    const color = '#6366f1';
    const remainderColor = document.body.classList.contains('dark-theme') ? '#334155' : '#edf2f7';
...
function updateMetricDOM() {
    ...
    document.getElementById('pb-completed').style.width = m.total > 0 ? `${(m.completed/m.total)*100}%` : '0%';
    document.getElementById('pb-pending').style.width = m.total > 0 ? `${(m.pending/m.total)*100}%` : '0%';
    document.getElementById('pb-progress').style.width = m.total > 0 ? `${(m.progress/m.total)*100}%` : '0%';
    document.getElementById('pb-high').style.width = m.total > 0 ? `${(m.high/m.total)*100}%` : '0%';
    ...
    if(document.getElementById('overview-daily')) document.getElementById('overview-daily').innerText = `${m.score}%`;
    if(document.getElementById('overview-weekly')) document.getElementById('overview-weekly').innerText = m.total > 0 ? `${Math.round((m.completed / m.total) * 90)}%` : '0%';
}
...
function renderTaskBoard() {
    ...
    todoList.innerHTML = ""; progressList.innerHTML = ""; completedList.innerHTML = "";
    let tCount = 0, pCount = 0, cCount = 0;

    taskState.forEach(task => {
        if (!task.title.toLowerCase().includes(searchFilterStr) || 
            (priorityFilterStr !== "" && task.priority !== priorityFilterStr) || 
            (statusFilterStr !== "all" && task.status !== statusFilterStr)) return;

        const card = document.createElement('div');
        const entrance = task.status === "Pending" ? "card-entry-left" : (task.status === "In Progress" ? "card-entry-bottom" : "card-entry-right");
        ...
        const cleanDate = isNaN(rawDate) ? task.date : rawDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
